...
api.register = async options => {
  const {did, ledger, mode} = options;
  const veresDriver = v1.driver({mode, hostname});

  let didDocument;

  if(did) {
    logger._debug(options, 'register DID', did);
    didDocument = await didStore({ledger, mode}).get(did);
  } else {
    didDocument = new VeresOneDidDoc({doc: JSON.parse(await getStdin())});
    logger._debug(
      options, 'register DID from stdin', {did: privateDidDocument.id});
  }

  return veresDriver.register({didDocument});
};
