...
async function _getLedger(did, hostname, reqOptions, options) {
  ...
  logger._debug(options, 'remote request', {url: didUrl});
  ...
  if(response.status !== 200) {
    logger._debug(options, 'remote failure response', {hostname, status, body});
    return {
      found: false,
      retry: true,
      timeMs,
  ...
  if(result.found && options.retrySimFailure !== 0.0 &&
    ...
    logger._debug(
      options, 'remote simulated failure response', {hostname, status});
    result.found = false;
    result.retry = true;
  } else {
    logger._debug(options, 'remote success response', {hostname, status});
  }
  return result;
}
