 ...
 */
function didStoreFor({type, mode}) {
  return {
 ...
 */
async function saveDidDocument({didDocument, status, type, mode, veresDriver}) {
  if(status === PENDING) {
    const did = didDocument.id;
    await didStore({type, mode, status: PENDING}).put(did, didDocument);
  } else { // REGISTERED
    await veresDriver.update({didDocument});
  }
}
