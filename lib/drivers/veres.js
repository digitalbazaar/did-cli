...
api.validate = async options => {
  const {did, document, contexts} = options;
  if(!did && !document) {
    throw new Error('validate requires either --did or --document');
  }
  if(did && document) {
    throw new Error('validate requires only one option --did or --document');
  }
  const input = await getStdin();
  if(did) {
    try {
      const identifier = input.trim();
      util.validateIdentifier(identifier);
    ...
    try {
      ...
      const jsons = contexts.split(' ').map(path => path ? require(path) : '');
      await util.validateDocument(json, jsons);
      console.log(`validated document ${json.id}`);
      process.exit(0);
    } catch(e) {
      console.error(e);
      process.exit(2);
