...
async function _send(options, didDocument, privateDidDocument) {
  ...
  if(response.status === 204) {
    _log(options, 'DID registration sent to the Veres One ledger!');
    _log(options, 'Please wait ~1 minute for the ledger to reach consensus.');
...
api['authn-add'] = async options => {
  _log(options, 'authn-add', options);
  const did = options.did;
  // lock local
  const release = await _lockDid(did, options);
  // get local did doc
  const didDocument = await keyStorage.load(did);
  // get remote did doc
  const hostname = _getHostname(options);
  const ledgerDidDocument = await didcv1.getObservable(
  ...
  let nextKeyId;
  do {
    nextKeyId = _nextKeyId(didDocument, suiteId);
  } while(_hasKeyId(ledgerDidDocument, nextKeyId));
  const params = _getParamsForType(didDocument, suiteId, options.type);
  const ledgerParams = _getParamsForType(
    ledgerDidDocument, suiteId, options.type);

  // public ledger key
  const newLedgerKey = {
    id: nextKeyId,
    owner: didDocument.id
  };
  ...
  //console.log('LEDGER', JSON.stringify(ledgerDidDocument, null, 2));
  //console.log('LOCAL', JSON.stringify(didDocument, null, 2));

  // FIXME: reusing import code, rename it
  _import(options, didDocument);

  const operation = didv1.wrap(
    {didDocument: ledgerDidDocument, operationType: 'update'});
  ...
  //if(options.send) {
    await _send(options, ledgerDidDocument, didDocument);
  //}
};
...
api['authn-remove'] = async options => {
  ...
  // FIXME: move most of this to ext library
  // remove key
  //console.log('LEDGER0', JSON.stringify(ledgerDidDocument, null, 2));
  //console.log('LOCAL0', JSON.stringify(didDocument, null, 2));
  _removeKeyId(didDocument, options.key);
  _removeKeyId(ledgerDidDocument, options.key);
  //console.log('LEDGER1', JSON.stringify(ledgerDidDocument, null, 2));
  //console.log('LOCAL1', JSON.stringify(didDocument, null, 2));
