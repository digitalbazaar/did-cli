...
async function _send(
  ...
  // get private key
  const privateKey = privateDidDocument.capabilityInvocation[0].publicKey[0]
    .privateKey;
 ...
 * @param [options.type] {string} 'ed25519' or 'rsa'
 * @param [options.passphrase] {string} Optional passphrase to encrypt keys with
 * @param [options.register] {boolean} Register DID Doc on ledger after generating?
 *
 * Params needed only if register == true:
 * @param [options.auth] {VeresOneDidDoc}
 * @param [options.accelerator] {string} hostname of accelerator to use
 *
 * Optional notes metadata:
 * @param [options.name] {string} DID Doc name
 * @param [options.description] {string} DID Doc description
 * @param [options.notes] {string}
 *
 * @param client {VeresOne}
 *
 * @returns {Promise}
 */
api.generate = async function generate(options, client) {
  _log(options, 'Generating a new Veres One DID...');
  _log(options, 'Generating keypair... (5-15 seconds)');

  const didDocument = await client.generate(options);
  ...
  await client.saveDoc(didDocument);

  _log(options, 'DID generation successful.');

  if(options.register) {
    _log(options, 'Registering DID Doc on ledger.');
    return client.register({didDocument, ...options});
  } else {
...
/**
 * Registers a DID Document on the ledger (it must already have been generated).
 *
 * @param options {object}
 * @param options.did {string} DID
 *
 * @param [options.auth] {VeresOneDidDoc}
 * @param [options.accelerator] {string} hostname of accelerator to use
 *
 ...
 */
api.register = async function register(options, client) {
  const {did} = options;
  _debug(options, 'register DID', {did});

  const didDocument = await client.get({did});
...
const _keyPrefixMap = {
  ...
  'capabilityDelegation': 'delegate-key-',
  'capabilityInvocation': 'invoke-key-'
};
...
/**
 * Adds a Service Endpoint to a given DID Document.
 *
 * @param options {object}
 * @param options.did {string} DID
 * @param options.name {string} Service name
 * @param [options.type] {string} Service type
 * @param [options.endpoint] {string} Service endpoint
 *
 * @param [options.update] {boolean}
 * @param [options.register] {boolean}
 * @param [options.auth] {VeresOneDidDoc}
 * @param [options.accelerator] {string} hostname of accelerator to use
 *
 * @param client {VeresOne} For example, `VeresOne` instance
 *
 * @returns {Promise}
 */
api['service-add'] = async function serviceAdd(options, client) {
  const {did, name, auth: authDoc, update} = options;
  _debug(options, 'service-add', {did: options.did});

  const didDocument = await client.get({did});

  if(didDocument.findService({name})) {
    if(update) {
      didDocument.removeService({name});
    } else {
      throw new Error(`Local service name exists: ${name}`);
    }
  }

  didDocument.addService({name, ...options});

  await client.saveDoc(didDocument); // save to local DID Doc cache

  if(options.register) {
    return client.register({didDocument, authDoc, ...options});
  }

  if(update) {
    return client.update({didDocument, authDoc, ...options});
  }
};

/**
 * Removes a Service Endpoint from a given DID Document.
 *
 * @param options {object}
 * @param options.did {string} DID
 * @param options.name {string} Service name
 ...
 * @returns {Promise}
 */
api['service-remove'] = async function serviceRemove(options, client) {
  const {did, name, auth: authDoc} = options;
  _debug(options, 'service-remove', {did, name});
  ...
  // todo: Is there a param that indicates whether to send update to ledger?
  return client.update({didDocument, authDoc, ...options});
};
