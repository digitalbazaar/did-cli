...
async function _send(
  ...
  if(operationType === 'create') {
    _log(options, 'Preparing to register DID on Veres One...');
  } else {
    _log(options, 'Preparing to update DID Document on Veres One...');
  }
  ...
  if(options.accelerator) {
    ...
    const response = await didcv1.sendToAccelerator({
      ...
      key: authDid.authentication[0].publicKey.privateKeyBase58 ||
        authDid.authentication[0].publicKey.privateKeyPem
    });
...
api['authn-remove'] = async options => {
  ...
  // FIXME: move most of this to ext library
  // remove key
  //console.log('LEDGER0', JSON.stringify(didDocument, null, 2));
  //console.log('LOCAL0', JSON.stringify(privateDidDocument, null, 2));
  _removeKeyId(privateDidDocument, options.key);
  _removeKeyId(didDocument, options.key);
  //console.log('LEDGER1', JSON.stringify(didDocument, null, 2));
