...
api.create = async options => {
  ...
  if(options.name || options.description) {
    ...
    await _notesSetMany(didDocument.id, properties, options);
  }
...
function _notesOne(did, config, options) {
  // run commands in order: clear, remove, set, get
  ...
  if(options.remove) {
    delete target[options.remove];
    store = true;
  }
  if(options.set) {
    if(options.set === 'id' || options.set === '@id') {
  ...
  if(options.find) {
    if(options.find[0] in target &&
      target[options.find[0]] === options.find[1]) {
      console.log(did);
    }
  }

  // if no options, show all notes
  if(!options.clear && !options.remove && !options.set && !options.get &&
    !options.find) {
...
async function _notesSetMany(did, properties, options) {
