...
function _nameToServiceId(didDocument, name) {
  return didDocument.id += ';' + encodeURIComponent(name);
}

// check for service with a name
function _hasService(didDocument, options) {
  const serviceId = _nameToServiceId(didDocument, options.name);
  const service = jsonld
    .getValues(didDocument, 'service')
    .find(service => service.id === serviceId);
  return service !== undefined;
}

function _addService(didDocument, options) {
  const serviceId = _nameToServiceId(didDocument, options.name);
  jsonld.addValue(didDocument, 'service', {
    id: serviceId,
    type: options.type,
...
function _removeService(didDocument, options) {
  const serviceId = _nameToServiceId(didDocument, options.name);
  const services = jsonld
    .getValues(didDocument, 'service')
    .filter(service => service.id !== serviceId);
  if(services.length === 0) {
    jsonld.removeProperty(didDocument, 'service');
  } else {
    didDocument.service = services;
  }
}

api['service-add'] = async options => {
  ...
  if(options.register) {
    try {
      didDocument = await didcv1.getObservable(
        {did, hostname, env: options.mode});
    } catch(e) {
      if(e.status !== 404) {
        throw e;
  ...
  // remove old local service
  if(_hasService(privateDidDocument, options)) {
    if(options.update) {
      // remove
      _removeService(privateDidDocument, options);
    } else {
      // err
      throw new Error(`local service name exists: ${options.name}`);
    }
  }

  // remove old remote service
  if(didDocument && _hasService(didDocument, options)) {
    if(options.update) {
      // remove
      _removeService(didDocument, options);
    } else {
      // err
      throw new Error(`remote service name exists: ${options.name}`);
    }
  }

  // FIXME: move most of this to ext library
  // add service
  _addService(privateDidDocument, options);
  if(didDocument) {
    _addService(didDocument, options);
  }
  ...
  if(options.register) {
    const operationType = didDocument ? 'update' : 'create';
    if(!didDocument) {
      didDocument = await didv1.publicDidDocument({privateDidDocument});
    }
    await _send({options, didDocument, privateDidDocument, operationType});
  }
};
