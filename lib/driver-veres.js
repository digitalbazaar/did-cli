...
api.create = options => {
  ...
  async.auto({
    ...
    pow: ['ledgerEvent', (results, callback) => equihashSigs.sign({
    ...
    if(results.postDidDocument.statusCode === 202) {
      // FIXME: do we want to deal with location header at all?
      const eventHash = results.postDidDocument.headers.location;
